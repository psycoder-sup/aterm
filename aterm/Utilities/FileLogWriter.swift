...
/// Writes log lines to `~/Library/Logs/aterm/aterm.log` with size-based rotation.
...
final class FileLogWriter: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.aterm.file-log-writer")
    ...
    private init() {
        ...
        logDirectory = home.appendingPathComponent("Library/Logs/aterm")
        logFileURL = logDirectory.appendingPathComponent("aterm.log")
        backupFileURL = logDirectory.appendingPathComponent("aterm.1.log")
        setupDirectory()
        openFile()
    }
