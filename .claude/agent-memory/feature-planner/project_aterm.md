...
Claude Session Status PRD v1.0 (Draft, 2026-04-07) at docs/feature/claude-session-status/claude-session-status-prd.md. Feature extends existing `status.set` IPC command with optional `--state` parameter for typed Claude Code session states (active, busy, idle, needs_attention, inactive). Pane-level tracking, priority-based bubble-up to Space level, Claude icon with color-coded indicator in sidebar. 6 Claude Code hooks map to state transitions. Key decisions: state and label are independent coexisting fields, no new IPC commands (extends status.set), no persistence across app restarts, aggregation at Space level only (not Workspace), no auto-focus on needs_attention in v1.
